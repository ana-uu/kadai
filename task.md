# ランキングサイト開発タスクリスト

## 全体タスク

- [ ] 開発環境の準備
  - [x] 既存サンプルの構造理解 :thinking:
  - [x] 仕様書の作成 :writing_hand:
  - [x] タスクリストの作成 :writing_hand:
  - [x] 開発用データの準備 :thinking:

## 1. FANZAランキングサイト

- [ ] バックエンド開発
  - [x] メインファイルの基本構造作成 :computer:
  - [x] サンプルデータの作成 :computer:
  - [x] 一覧表示ルート実装 :computer:
  - [x] 詳細表示ルート実装 :computer:
  - [x] 新規登録フォームルート実装 :computer:
  - [x] 新規登録処理ルート実装 :computer:

- [ ] フロントエンド開発
  - [x] 一覧ページ作成 :writing_hand:
  - [x] 詳細ページ作成 :writing_hand:
  - [x] 新規登録フォーム作成 :writing_hand:

- [ ] テスト・デバッグ
  - [ ] 一覧表示の動作確認 :computer:
  - [ ] 詳細表示の動作確認 :computer:
  - [ ] 新規登録の動作確認 :computer:
  - [ ] エラーハンドリングの確認 :computer:

## 2. Amazonランキングサイト

- [ ] バックエンド開発
  - [ ] メインファイルの基本構造作成 :computer:
  - [ ] サンプルデータの作成 :computer:
  - [ ] 一覧表示ルート実装 :computer:
  - [ ] 詳細表示ルート実装 :computer:
  - [ ] 新規登録フォームルート実装 :computer:
  - [ ] 新規登録処理ルート実装 :computer:

- [ ] フロントエンド開発
  - [ ] 一覧ページ作成 :writing_hand:
  - [ ] 詳細ページ作成 :writing_hand:
  - [ ] 新規登録フォーム作成 :writing_hand:

- [ ] テスト・デバッグ
  - [ ] 一覧表示の動作確認 :computer:
  - [ ] 詳細表示の動作確認 :computer:
  - [ ] 新規登録の動作確認 :computer:
  - [ ] エラーハンドリングの確認 :computer:

## 3. 観光地ランキングサイト

- [ ] バックエンド開発
  - [ ] メインファイルの基本構造作成 :computer:
  - [ ] サンプルデータの作成 :computer:
  - [ ] 一覧表示ルート実装 :computer:
  - [ ] 詳細表示ルート実装 :computer:
  - [ ] 新規登録フォームルート実装 :computer:
  - [ ] 新規登録処理ルート実装 :computer:

- [ ] フロントエンド開発
  - [ ] 一覧ページ作成 :writing_hand:
  - [ ] 詳細ページ作成 :writing_hand:
  - [ ] 新規登録フォーム作成 :writing_hand:

- [ ] テスト・デバッグ
  - [ ] 一覧表示の動作確認 :computer:
  - [ ] 詳細表示の動作確認 :computer:
  - [ ] 新規登録の動作確認 :computer:
  - [ ] エラーハンドリングの確認 :computer:

## 最終確認・提出

- [ ] 全体テスト
  - [ ] 3つのサイトが異なるポートで独立動作することを確認 :computer:
  - [ ] 各サイトのルートパスが正しく設定されていることを確認 :computer:
  - [ ] 各サイトのCRUD機能が正常動作することを確認 :computer:

- [ ] ドキュメント整備
  - [ ] README.mdの更新 :writing_hand:
  - [ ] 各サイトの起動方法の記載 :writing_hand:
  - [ ] アクセスURLの記載 :writing_hand:
  - [ ] 使用方法の説明追加 :writing_hand:

- [ ] 提出準備
  - [ ] コードの最終チェック :eyes:
  - [ ] 不要なファイルの削除 :computer:
  - [ ] 提出 :tada:

## 注意事項

- 各サイトは完全に独立したファイルとして作成
- 各サイトは異なるポートで動作
- 既存サンプルの構造を参考にしつつ、各ランキングの特性に合わせてカスタマイズ
- データ構造は仕様書に従って実装
- エラーハンドリングを適切に実装
- 各サイトは同時に起動可能